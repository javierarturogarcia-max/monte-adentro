 ...
 */
/**
 * La casa, en sus ocho niveles. Es lo que se ve crecer y es el premio de todo
 * el juego, asi que cada escalon tiene que notarse desde lejos: primero cambia
 * la silueta (corredor, troje, cuartos), luego el material (cana, bahareque,
 * adobe) y al final el techo (palma, teja).
 *
 * Es una casa donde viven nueve. Grande de verdad, no una caseta: seis metros
 * de frente en la choza y nueve cuando ya es casa propia.
 */
export function choza(nivel = 1) {
  ...
  const troje = n >= 3;
  const cuartos = n >= 7;
  const ancho = n >= 7 ? 8.6 : n >= 5 ? 7.4 : n >= 3 ? 6.6 : 6;
  const fondo = n >= 7 ? 6.2 : n >= 5 ? 5.4 : 4.8;
  const alto = n >= 5 ? 2.6 : n >= 4 ? 2.4 : 2.2;
  const cumbre = alto + (n >= 6 ? 1.5 : 1.75);   // la palma va mas empinada
  const horcon = [0.42, 0.33, 0.22];
  const cana = [0.62, 0.52, 0.33];

  /**
   * Un techo de dos aguas sobre un rectangulo: caballete, pares a cada lado y
   * encima la cubierta. Es lo que hace que se lea "casa" y no "mesa".
   */
  const dosAguas = (c, { anchoT, fondoT, base, alturaT, en = [0, 0, 0], material }) => {
    const alero = 0.7;
    const a = anchoT + alero * 2, f = fondoT / 2 + alero;
    const pendiente = Math.atan2(alturaT, f);
    const largo = Math.hypot(alturaT, f);
    const [ex, ey, ez] = en;

    // Caballete de punta a punta.
    c.cilindro({ radio: 0.09, alto: a, lados: 6, giroY: Math.PI / 2, inclina: Math.PI / 2,
      en: [ex - a / 2, ey + base + alturaT, ez], color: horcon });
    // Tijeras: los pares que sostienen el techo, visibles en los extremos.
    for (const sx of [-1, 1]) {
      for (const sz of [-1, 1]) {
        c.cilindro({ radio: 0.055, alto: largo, lados: 4,
          en: [ex + sx * anchoT / 2, ey + base, ez + sz * fondoT / 2],
          inclina: sz * (Math.PI / 2 - pendiente), giroY: 0, color: horcon });
      }
    }

    if (material === 'palma') {
      // Manojos de palma tendidos sobre las dos vertientes, en tres hiladas
      // para que se vea la hoja y no una plancha.
      for (const sz of [-1, 1]) {
        for (let hilada = 0; hilada < 3; hilada++) {
          const t = 0.2 + hilada * 0.31;
          const y = ey + base + alturaT * (1 - t);
          const z = ez + sz * f * t;
          for (let k = 0; k < 15; k++) {
            const x = ex - a / 2 + ((k + 0.5) / 15) * a;
            c.hoja({ largo: largo * 0.52, ancho: 0.62, curva: 0.24, tramos: 3,
              en: [x, y, z], giroY: sz > 0 ? 0 : Math.PI,
              // La hoja crece hacia +Y: hay que pasarla de la vertical y seguir
              // bajando lo que baja la vertiente, o quedan pinchos de punta.
              inclina: Math.PI / 2 + pendiente,
              color: tinte(PALETA.paja, ((k + hilada) % 3) * 0.09 - 0.06) });
          }
        }
      }
    } else {
      // Teja: dos planchas y el caballete encima, en hiladas para que la luz
      // les saque el relieve.
      for (const sz of [-1, 1]) {
        for (let hilada = 0; hilada < 3; hilada++) {
          const t = (hilada + 0.5) / 3;
          c.caja({ ancho: a, alto: 0.1, fondo: (f / 3) * 1.08,
            en: [ex, ey + base + alturaT * (1 - t) + 0.05, ez + sz * f * t],
            inclina: sz * pendiente,
            color: tinte(PALETA.teja, hilada * 0.05 - 0.05) });
        }
      }
      c.caja({ ancho: a + 0.12, alto: 0.16, fondo: 0.34,
        en: [ex, ey + base + alturaT - 0.02, ez], color: [0.44, 0.20, 0.14] });
    }
  };

  return construir(`choza_${n}`, (c) => {
    // --- horcones: los cuatro palos que aguantan todo. Nunca se van.
    for (const sx of [-1, 1]) {
      for (const sz of [-1, 1]) {
        c.cilindro({ radio: 0.13, alto, lados: 6,
          en: [sx * ancho / 2, 0, sz * fondo / 2], color: horcon });
      }
    }
    // Un horcon mas a media pared cuando la casa ya es larga.
    if (n >= 3) for (const sz of [-1, 1]) {
      c.cilindro({ radio: 0.11, alto, lados: 6, en: [0, 0, sz * fondo / 2], color: horcon });
    }

    // --- paredes
    if (paredes === 'abierta') {
      // Cana rajada de horcon a horcon: entra el aire y se ve el fuego de fuera.
      const puerta = 0.75;
      for (const sz of [-1, 1]) {
        for (let k = 0; k < 17; k++) {
          const x = -ancho / 2 + (k / 16) * ancho;
          if (sz > 0 && Math.abs(x) < puerta) continue;
          c.cilindro({ radio: 0.04, alto: alto * 0.94, lados: 4,
            en: [x, 0, sz * fondo / 2], color: tinte(cana, (k % 3) * 0.05 - 0.05) });
        }
      }
      for (const sx of [-1, 1]) {
        for (let k = 1; k < 13; k++) {
          c.cilindro({ radio: 0.04, alto: alto * 0.94, lados: 4,
            en: [sx * ancho / 2, 0, -fondo / 2 + (k / 13) * fondo],
            color: tinte(cana, (k % 3) * 0.05 - 0.05) });
        }
      }
      // Dos travesanos que amarran la cana.
      for (const y of [alto * 0.35, alto * 0.75]) {
        for (const sz of [-1, 1]) {
          c.cilindro({ radio: 0.05, alto: ancho, lados: 4, giroY: Math.PI / 2, inclina: Math.PI / 2,
            en: [-ancho / 2, y, sz * fondo / 2], color: horcon });
        }
      }
    } else {
      const color = paredes === 'adobe' ? PALETA.adobe : [0.52, 0.41, 0.29];
      c.caja({ ancho, alto, fondo, color });
      if (paredes === 'bahareque') {
        // Bahareque: barro sobre varas, y las varas se siguen viendo.
        for (let k = 0; k < 9; k++) {
          c.cilindro({ radio: 0.045, alto: alto * 0.9, lados: 4,
            en: [-ancho / 2 + ((k + 0.5) / 9) * ancho, 0, fondo / 2 + 0.02],
            color: tinte(cana, -0.08) });
        }
      }
      // El hueco de la puerta.
      c.caja({ ancho: 1.15, alto: alto * 0.82, fondo: 0.14,
        en: [0, 0, fondo / 2 + 0.02], color: [0.24, 0.16, 0.11] });
      if (n >= 5) {
        for (const sx of [-1, 1]) {
          c.caja({ ancho: 0.8, alto: 0.7, fondo: 0.14,
            en: [sx * ancho * 0.3, alto * 0.48, fondo / 2 + 0.02], color: [0.14, 0.14, 0.16] });
          // Marco de madera: con el adobe ya hay ventanas de verdad.
          c.caja({ ancho: 0.92, alto: 0.08, fondo: 0.1,
            en: [sx * ancho * 0.3, alto * 0.48 + 0.7, fondo / 2 + 0.03], color: horcon });
        }
      }
    }

    // --- el suelo: tierra apisonada hasta que hay adobe
    c.rejilla({ ancho: ancho + 0.5, fondo: fondo + 0.5, divX: 2, divZ: 2,
      alturaEn: () => 0.04, color: n >= 5 ? [0.62, 0.56, 0.48] : PALETA.tierra });

    // --- el techo
    dosAguas(c, { anchoT: ancho, fondoT: fondo, base: alto, alturaT: cumbre - alto,
      material: techo });

    // --- corredor delante: el alero donde se está cuando llueve
    if (corredor) {
      const largo = n >= 5 ? 2.6 : 2.1;
      const altoC = alto * 0.95;
      for (const x of [-ancho * 0.42, 0, ancho * 0.42]) {
        c.cilindro({ radio: 0.1, alto: altoC, lados: 5,
          en: [x, 0, fondo / 2 + largo], color: horcon });
      }
      c.cilindro({ radio: 0.07, alto: ancho + 0.6, lados: 5, giroY: Math.PI / 2, inclina: Math.PI / 2,
        en: [-(ancho + 0.6) / 2, altoC, fondo / 2 + largo], color: horcon });
      const caida = Math.atan2(alto + 0.5 - altoC, largo);
      if (techo === 'palma') {
        for (let hilada = 0; hilada < 2; hilada++) {
          for (let k = 0; k < 13; k++) {
            const t = 0.25 + hilada * 0.45;
            c.hoja({ largo: largo * 0.72, ancho: 0.58, curva: 0.2, tramos: 2,
              en: [-ancho / 2 + ((k + 0.5) / 13) * ancho,
                alto + 0.5 - (alto + 0.5 - altoC) * t, fondo / 2 + largo * t],
              inclina: Math.PI / 2 + caida,
              color: tinte(PALETA.paja, ((k + hilada) % 3) * 0.08 - 0.05) });
          }
        }
      } else {
        c.caja({ ancho: ancho + 0.6, alto: 0.11, fondo: largo + 0.4,
          en: [0, alto + 0.28, fondo / 2 + largo / 2], inclina: caida, color: PALETA.teja });
      }
      c.rejilla({ ancho: ancho + 0.3, fondo: largo + 0.4, divX: 2, divZ: 2,
        alturaEn: () => 0.06, en: [0, 0, fondo / 2 + largo / 2], color: [0.55, 0.48, 0.40] });
    }

    // --- troje: la mazorca en alto, sobre cuatro patas, para que no se pique
    if (troje) {
      const tx = ancho / 2 + 2.3;
      const patas = 1.15;
      for (const sx of [-1, 1]) {
        for (const sz of [-1, 1]) {
          c.cilindro({ radio: 0.09, alto: patas, lados: 4,
            en: [tx + sx * 0.75, 0, sz * 0.75], color: horcon });
        }
      }
      // Varas verticales: el troje es de varas, se ve la mazorca por dentro.
      for (let k = 0; k < 12; k++) {
        const a = (k / 12) * Math.PI * 2;
        c.cilindro({ radio: 0.04, alto: 1.3, lados: 4,
          en: [tx + Math.cos(a) * 0.95, patas, Math.sin(a) * 0.95], color: cana });
      }
      c.caja({ ancho: 1.9, alto: 0.12, fondo: 1.9, en: [tx, patas, 0], color: [0.50, 0.40, 0.26] });
      dosAguas(c, { anchoT: 1.7, fondoT: 1.7, base: patas + 1.3, alturaT: 0.75,
        en: [tx, 0, 0], material: 'palma' });
    }

    // --- los cuartos: la casa deja de ser una sola pieza
    if (cuartos) {
      const cx = -(ancho / 2 + 1.9);
      c.caja({ ancho: 3.6, alto, fondo: fondo * 0.86, en: [cx, 0, 0], color: PALETA.adobe });
      c.caja({ ancho: 0.95, alto: alto * 0.8, fondo: 0.14,
        en: [cx, 0, fondo * 0.43 + 0.02], color: [0.24, 0.16, 0.11] });
      c.caja({ ancho: 0.7, alto: 0.62, fondo: 0.14,
        en: [cx - 1.1, alto * 0.48, fondo * 0.43 + 0.02], color: [0.14, 0.14, 0.16] });
      dosAguas(c, { anchoT: 3.6, fondoT: fondo * 0.86, base: alto, alturaT: 1.0,
        en: [cx, 0, 0], material: techo });
    }
  });
}
